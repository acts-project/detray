...
int main(int, char**) {
    ...
    detray::mask<detray::annulus2D<>> ann2D{0u,  100.0, 200.0, -1.0,
                                            1.0, 0.0,   0.0,   100.0};
    const auto ann2D_proto = detray::svgtools::conversion::surface<point3_container>(transform, ann2D);     
    ...
    const auto cyl2D_proto = detray::svgtools::conversion::surface<point3_container>(transform, cyl2D);     
    ...
    const auto rec2D_proto = detray::svgtools::conversion::surface<point3_container>(transform, rec2D);     
    ...
    const auto rin2D_proto = detray::svgtools::conversion::surface<point3_container>(transform, rin2D);     
    ...
    const auto tra2D_proto = detray::svgtools::conversion::surface<point3_container>(transform, tra2D);     

...
int main(int, char**) {
    ...
    // In this test we will create a web page to show the detector geometry and more.
    // We will start by creating the svgs we want to include in the web page.
    ...
    // Draw some example trajectories and include them in the svg vector (along with their intersections).
    for (const auto qop : std::vector{-4, -8, -16}){
        ...
        const auto svg_helix = il.draw_trajectory(name + "_trajectory", helix, view);

        // Draw the intersection record.
        const auto svg_helix_ir =
            il.draw_intersections(name + "_record", helix_ir, view);

        // We one the trajectory and intersection record to be considered as one svg. 
        // Thus we group them together before adding the group to the svg vector.
        const auto svg_group = detray::svgtools::utils::group(name, std::vector{svg_helix, svg_helix_ir});
    ...
    // To visualize the svg objects in a specific order we need to pass a comparator before we build the page.
    // For instance we might want to display helices on top on the volumes (rather than the other way around).
    // In this example we will alphanumeric_compare which renderes the svgs such that the id of the svg object
    // with the greatest id alphanumerically will be displayed on top.
    // Build the web page.
    auto alphanum_cmp = actsvg::web::compare::alphanumeric;
    builder.build(current_directory / "test_svgtools_website", svgs, alphanum_cmp);

    // Once the direcroy has been created, run the server using "python3 -m http.server" in the directory.
    // Subsequently connect to localhost using the respective port.
    // On the web page, click and drag to move the view box. Use the scroll wheel to zoom.
}

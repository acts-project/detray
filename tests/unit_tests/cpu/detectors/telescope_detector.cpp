...
GTEST_TEST(detray_detectors, telescope_detector) {
    ...
    while (heartbeat_z1 && heartbeat_z2 && heartbeat_x) {
        ...
        heartbeat_z1 &= rk_stepper_z.step(navigation_z1(), do_reset_z1,
                                          stepping_z1, prop_cfg.stepping);
        heartbeat_z2 &= rk_stepper_z.step(navigation_z2(), do_reset_z2,
                                          stepping_z2, prop_cfg.stepping);
        heartbeat_x &= rk_stepper_x.step(navigation_x(), do_reset_x, stepping_x,
                                         prop_cfg.stepping);
    ...
    while (heartbeat_tel) {
        const bool do_reset_tel{navigation_z1.is_on_surface() ||
                                navigation_z1.is_init()};
        heartbeat_tel &= rk_stepper_z.step(tel_navigation(), do_reset_tel,
                                           tel_stepping, prop_cfg.stepping);

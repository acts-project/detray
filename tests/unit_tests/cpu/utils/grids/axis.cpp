...
GTEST_TEST(detray_grid, axis_comparison) {
    ...
    auto test_axis = [](auto axis) -> void {
        ...
        vecmem::vector<scalar> different_bin_edges = {-10.f, -8.f, -3.f, 7.f,
                                                      7.f,   14.f, 20.f};
        ...
        // An open regular x-axis
        axis_type or_axis{edge_range, &bin_edges};

        axis_type another_or_axis{edge_range, &bin_edges};

        axis_type differnt_or_axis{edge_range, &different_bin_edges};

        axis_type differnt_edges_range_or_axis{different_edge_range,
                                               &bin_edges};

        // Test if the axes are equal
        EXPECT_EQ(or_axis, another_or_axis);

        // Confirm that the axes are not equal
        EXPECT_NE(or_axis, differnt_or_axis);

        // Confirm that the axes are not equal
        EXPECT_NE(or_axis, differnt_edges_range_or_axis);
    };

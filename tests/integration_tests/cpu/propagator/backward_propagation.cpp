...
TEST_P(BackwardPropagation, backward_propagation) {
    ...
    EXPECT_TRUE(getter::element(bound_cov2, e_bound_qoverp, e_bound_qoverp) >
                getter::element(bound_cov1, e_bound_qoverp, e_bound_qoverp));
    EXPECT_TRUE(getter::element(bound_cov2, e_bound_theta, e_bound_theta) >
                getter::element(bound_cov1, e_bound_theta, e_bound_theta));

    std::cout << getter::element(bound_cov2, e_bound_phi, e_bound_phi) << "  "
              << getter::element(bound_cov1, e_bound_phi, e_bound_phi)
              << std::endl;

    EXPECT_TRUE(getter::element(bound_cov2, e_bound_phi, e_bound_phi) >
                getter::element(bound_cov1, e_bound_phi, e_bound_phi));
}

...
class BackwardPropagationTelescope
    : public ::testing::TestWithParam<std::tuple<std::vector<scalar>, scalar>> {
};

TEST_P(BackwardPropagationTelescope, backward_propagation_telescope) {
...
INSTANTIATE_TEST_SUITE_P(
    telescope, BackwardPropagationTelescope,
    ::testing::Values(
...
GTEST_TEST(BackwardPropagationToy, BackwardPropagationToy) {
    ...
    propagation::config prop_cfg{};
    prop_cfg.stepping.rk_error_tol = 1e-4f * unit<float>::mm;
    prop_cfg.navigation.overstep_tolerance = -100.f * unit<float>::um;
    propagator_t p{prop_cfg};
    ...
    */
    ...
    bw_state.do_debug = true;
    bw_state._navigation.set_direction(navigation::direction::e_backward);
    bw_state._navigation.set_volume(bound_param1.surface_link().volume());

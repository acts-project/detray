...
inline auto run_navigation_validation(
    ...
    hip_cpy.setup(recorded_intersections_buffer)->wait();
    ...
    hip_cpy(recorded_intersections_buffer, recorded_intersections)->wait();
...
class navigation_validation : public test::fixture_base<> {
    ...
    explicit navigation_validation(
        ...
        m_truth_data_name = k_use_rays ? det_name + "_ray_scan_for_hip"
                                       : det_name + "_helix_scan_for_hip";
    ...
    void TestBody() override {
        ...
        const auto truth_trk_path{make_path("truth_track_params_hip")};
        const auto trk_path{make_path("navigation_track_params_hip")};
        const auto truth_intr_path{make_path("truth_intersections_hip")};
        const auto intr_path{make_path("navigation_intersections_hip")};
        const auto mat_path{make_path("accumulated_material_hip")};
        const auto missed_path{make_path("missed_intersections_dists_hip")};
    ...
    vecmem::hip::device_memory_resource m_dev_mr{};
...
using straight_line_navigation =
    detray::hip::navigation_validation<detector_t, detray::ray_scan>;

template <typename detector_t>
using helix_navigation =
    detray::hip::navigation_validation<detector_t, detray::helix_scan>;

    ...
    if (is_cyl) {
        ...
        for (const auto& mat_slab : mat_map.all()) {
            EXPECT_TRUE(mat_slab.get_material() ==
                        toy_config<scalar>{}.mapped_material());
        }
    } else {
...
inline bool toy_detector_test_new(
    ...
    EXPECT_EQ(names.at(0u), "toy_detector_new");
    ...
    auto portal_mat = material_slab<scalar_t>(
        toy_config<scalar_t>{}.mapped_material(), 1.5f * unit<scalar_t>::mm);

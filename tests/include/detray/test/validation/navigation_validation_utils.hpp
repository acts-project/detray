    ...
    for (long i = 0; i < static_cast<long>(max_entries); ++i) {
        ...
        if (!found_same_surfaces) {
            ...
            auto compare_and_equalize =
                [&i, &handle_counting_error, &is_swapped_portals,
                 &missed_intersections]<typename trace_t,
                    ...
                    trace_t &trace, typename trace_t::iterator last_missed,
                    ...
                    const auto n_check{
                        std::distance(first_missed, last_missed)};
                    assert(n_check > 0);
            ...
            if (is_swapped_portals(i)) {
                ...
                ++i;
            } else if (auto last_missed = std::ranges::find_if(
                           std::ranges::begin(truth_trace) + i,
                           std::ranges::end(truth_trace), search_nav_on_truth);
                       last_missed != std::end(truth_trace)) {

                // The navigator missed a(multiple) surface(s)
                compare_and_equalize(truth_trace, last_missed, recorded_trace,
                                     n_miss_nav, missed_pt_nav, missed_sn_nav,
                                     missed_ps_nav);
                           ...
                           search_truth_on_nav);
                       last_missed != std::end(recorded_trace)) {

                // The navigator found a(multiple) extra surface(s)
                compare_and_equalize(recorded_trace, last_missed, truth_trace,
                                     n_miss_truth, missed_pt_tr, missed_sn_tr,
                                     missed_ps_tr);
...
auto compare_to_navigation(
    ...
    for (std::size_t i = 0u; i < n_samples; ++i) {
        ...
        auto [success, obj_tracer, step_trace, mat_record, mat_trace,
            ...
            record_propagation<stepper_t, actor_ts...>(
                ctx, &host_mr, det, prop_cfg, track, field_view);
    ...
    std::cout << std::left << std::setw(cw + 5)
              ...
              << " (Mean per track: "
              << static_cast<double>(n_miss_nav.n_sensitives) / n_tracks << ")"
              << std::endl;
    std::cout << std::left << std::setw(cw + 5)
              << "Max no. of add. surfaces per track: " << n_max_extra_per_trk
              << " (Mean per track: "
              << static_cast<double>(n_miss_truth.n_total()) / n_tracks << ")"
              << std::endl;
    std::cout << "\n-----------------------------------" << std::endl;

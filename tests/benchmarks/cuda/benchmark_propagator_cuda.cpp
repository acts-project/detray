...
static void BM_PROPAGATOR_CPU(benchmark::State &state) {
    ...
    for (auto _ : state) {
        ...
        for (auto &track : tracks) {
            ...
            propagator_host_type<stepper_policy_t>::state p_state(track);
...
BENCHMARK(BM_PROPAGATOR_CUDA<stepper_default_policy>)
    ->RangeMultiplier(2)
    ->Range(8, 256);
BENCHMARK(BM_PROPAGATOR_CPU<stepper_rk_correction_policy>)
    ->RangeMultiplier(2)
    ->Range(8, 256);
BENCHMARK(BM_PROPAGATOR_CUDA<stepper_rk_correction_policy>)
    ->RangeMultiplier(2)
    ->Range(8, 256);

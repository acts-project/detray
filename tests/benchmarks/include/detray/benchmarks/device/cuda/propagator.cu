...
struct propagation_queue {
    ...
    DETRAY_DEVICE propagation_queue(
        data_buffer_type &data, cooperative_groups::thread_group &thread_group,
...
__global__ void __launch_bounds__(256, 4) propagation_kernel(
    ...
    std::array<propagation_state_t, 2u> propagation{
        propagation_state_t{stepper_state_t{tracks.at(2 * gid), field_view},
                            navigation_state_t{*pinned_detector_ptr}, gctx},
        propagation_state_t{stepper_state_t{tracks.at(2 * gid + 1), field_view},
                            navigation_state_t{*pinned_detector_ptr}, gctx}};
    ...
    for (int i = 0; i < 2; ++i) {
        ...
        // Update the navigation information, in case the actors changed the
        // track
    ...
    cooperative_groups::thread_group warp =
        cooperative_groups::tiled_partition<32>(
            cooperative_groups::this_thread_block());

    if (block.thread_rank() < blockDim.x / 2) {
        DETRAY_INFO_HOST_DEVICE("Thread %d (%d) in block %d doing stepping",
                                gid, block.thread_rank(), blockIdx.x);

        propagation_queue<navigator_t, stepper_t, actor_chain_t> queue{
            propagation, warp, barriers};

        take_step(step_cfg, queue, gid);
    } else {
        DETRAY_INFO_HOST_DEVICE("Thread %d (%d) in block %d doing navigation",
                                gid, block.thread_rank(), blockIdx.x);
...
void run_propagation_kernel(
    ...
    int block_dim = static_cast<int>((tracks_view.size() / 2 + thread_dim - 1) /
                                     thread_dim);
    ...
    propagation_kernel<navigator_t, stepper_t, actor_chain_t>
        <<<block_dim, thread_dim>>>(pinned_cfg, pinned_detector_ptr, field_view,
                                    tracks_view, pinned_actor_state_ptr);

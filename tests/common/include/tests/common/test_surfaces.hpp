 ...
 */
dvector<surface_base<transform3>>
...
create_barrel_components(scalar r, scalar stagger_r, unsigned int n_phi,
  ...
  for (unsigned int iz = 0; iz < n_z; ++iz) {
    ...
    for (unsigned int iphi = 0; iphi < n_phi; ++iphi) {
      scalar phi = -M_PI + iphi * step_phi;
      // Populate the grids
      barrel_grid_inner.populate(cylinder_point2{volume_inner_r * phi, pos_z},
                                 transforms.size() + transform_offset);
      barrel_grid_outer.populate(cylinder_point2{volume_outer_r * phi, pos_z},
                                 transforms.size() + transform_offset);
      if (iz == 0) {
      ...
      point3 p = {(r + r_addon) * std::cos(phi), (r + r_addon) * std::sin(phi),
                  pos_z};

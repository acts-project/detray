...
TEST(tools, intersection_kernel_single) {
  ...
  auto within_epsilon = [](const point3 &a, const point3 &b,
    ...
    return (std::abs(a[0] - b[0]) < epsilon &&
            std::abs(a[1] - b[1]) < epsilon && std::abs(a[2] - b[2]) < epsilon);
  };
  ...
  ASSERT_TRUE(
      within_epsilon(std::get<0>(sfi_rectangle).p3, expected_rectangle, 1e-7));
  ...
  ASSERT_TRUE(
      within_epsilon(std::get<0>(sfi_trapezoid).p3, expected_trapezoid, 1e-7));

  auto sfi_annulus =
      intersect_by_group(track, annulus_transform, std::get<2>(mask_store), 0);

  point3 expected_annulus{0.03, 0.03, 30.};
  ASSERT_TRUE(
      within_epsilon(std::get<0>(sfi_annulus).p3, expected_annulus, 1e-7));

  std::vector<point3> expected_points = {expected_rectangle, expected_trapezoid,
                                         expected_annulus};

...
TEST(ALGEBRA_PLUGIN, volume) {
    ...
    v1.template set_range<e_surface>(surface_range);
    v1.template set_range<e_portal>(portal_range);
    ASSERT_TRUE(v1.template range<e_surface>() == surface_range);
    ASSERT_TRUE(v1.template range<e_portal>() == portal_range);
    ASSERT_FALSE(v1.empty());
    ASSERT_EQ(v1.template n_objects<e_surface>(), 6);
    ASSERT_EQ(v1.template n_objects<e_portal>(), 4);
    ...
    ASSERT_TRUE(v2.template range<e_surface>() == surface_range);
    ASSERT_TRUE(v2.template range<e_portal>() == portal_range);
}

...
struct annulus2 {
  ...
  is_inside(const point2 &p, const mask_tolerance &t = within_epsilon) const {
    ...
    if constexpr (std::is_same_v<inside_local_type, __plugin::cartesian2>) {
      ...
      return (phi_strp >= _values[2] - t[1] and phi_strp <= _values[3] + t[1])
                 ? e_inside
                 : e_outside;
    }
    // polar strip coordinates given
      ...
      return (r_mod2 >= minR_tol * minR_tol and r_mod2 <= maxR_tol * maxR_tol)
                 ? e_inside
                 : e_outside;
    }
  }
   ...
   *
   **/
  bool operator==(const array_type<scalar, 7> &rhs) { return (_values == rhs); }

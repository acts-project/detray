...
namespace detray
{
    ...
    struct annulus2
    {
        ...
        {
            // transcription from ACTS AnnulusBounds
            
            // For a point p in local polar coordinates, rotate by avr phi
            scalar_type p_phi = p[1] - _values[6];

            // Check phi boundaries, which are well def. in local frame
            if (p_phi < _values[2] - t1 || p_phi > _values[3] + t1) return e_outside;
            ...
            scalar_type r_mod2 = shift_r * shift_r + p[0] * p[0] + 
                                 2 * shift_r * p[0] * std::cos(p_phi - shift_phi);

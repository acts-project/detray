...
struct intersection_kernel {
    ...
    template <typename track_type, typename mask_container, typename mask_range,
              std::size_t first_mask_id, std::size_t... remaining_mask_ids>
    inline auto unroll_intersect(
        const track_type &track, const transform3 &ctf,
        const mask_container &masks, const mask_range &rng,
        const std::size_t mask_id, dindex volume_index,
        std::integer_sequence<std::size_t, first_mask_id, remaining_mask_ids...>
        /*available_ids*/) {
        ...
        std::integer_sequence<std::size_t, remaining_mask_ids...> remaining;

        // Unroll as long as you have at least 1 entries
        if constexpr (remaining.size() >= 1) {
            return (unroll_intersect(track, ctf, masks, rng, mask_id,
    ...
    inline const auto operator()(
        const surface_type &surface, const track_type &track,
        ...
        return unroll_intersect(
            ...
            std::make_integer_sequence<
                std::size_t,
                detail::tuple_size<
                    typename mask_container::mask_tuple>::value>{});
    }
};
}  // namespace detray
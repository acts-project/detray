...
class navigator {
    ...
    DETRAY_HOST_DEVICE inline intersection update_next_candidate(
        ...
        auto sfi =
             _detector->surfaces()[obj_idx].intersect(track,
    ...
    DETRAY_HOST_DEVICE inline void initialize_kernel(
        ...
        // Loop over surfaces in volume
        for (const auto [obj_idx, obj] : enumerate(
                 _detector->surfaces(),
                 volume.neighborhood(track, _detector->sf_finders_store()))) {
            // Retrieve candidate from the object
            auto sfi = obj.intersect(track, _detector->transform_store(),

...
struct surface_kernels {
    ...
    struct get_volume_links {
        template <typename mask_group_t, typename idx_range_t>
        DETRAY_HOST constexpr auto operator()(
    ...
    struct get_mask_value {
        template <typename mask_group_t, typename idx_range_t>
        DETRAY_HOST constexpr auto operator()(const mask_group_t& mask_group,
    ...
    struct get_mask_values {
        ...
        DETRAY_HOST constexpr auto operator()(
            ...
            for (const auto& mask :
                 detray::ranges::subrange(mask_group, idx_range)) {
                values.emplace_back();
                for (const scalar_type v : mask.values()) {
                    values.back().push_back(v);
                }
            }
    ...
    struct centroid {
        ...
        DETRAY_HOST_DEVICE constexpr point3_type operator()(
            ...
            // The middle index is the best guess
            const dindex middle_idx{idx_range.upper() - idx_range.lower() / 2};

            return mask_group[idx_range.lower() + middle_idx].centroid();
        }
    };

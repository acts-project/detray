...
class rk_stepper final
    ...
    struct state : public base_type::state {
        ...
        scalar_type d2qopdsdqop(
            ...
            const scalar_type g =
                -1.f * I.compute_stopping_power(
                           *vol_mat_ptr, this->particle_hypothesis(), rq);
    ...
    DETRAY_HOST_DEVICE bool step(
        ...
        DETRAY_VERBOSE_HOST_DEVICE("Initial stepsize: %f mm",
                                   stepping.step_size());
        ...
        detray::tie(sd.dtds[0u], sd.t[0u]) =
            evaluate_dtds(stepping, sd.b_first, 0u, 0.f,
        ...
        const auto estimate_error = [&](const scalar_type& h) {
            ...
            DETRAY_DEBUG_HOST_DEVICE("-> B-field: [%f, %f, %f]", bvec[0],
                                     bvec[1], bvec[2]);
            ...
            detray::tie(sd.dqopds[3u], sd.qop[3u]) = evaluate_dqopds(
                stepping, 3u, h, sd.dqopds[2u], vol_mat_ptr, cfg);
            detray::tie(sd.dtds[3u], sd.t[3u]) = evaluate_dtds(
                stepping, sd.b_last, 3u, h, sd.dtds[2u], sd.qop[3u]);
        ...
        const auto step_size_scaling =
            ...
            return static_cast<scalar_type>(math::min(
                math::max(math::sqrt(math::sqrt(cfg.rk_error_tol / err)),
        ...
        for (unsigned int i = 0u; i < n_trials; i++) {
            ...
            // ---> Make step size smaller and estimate error again
                ...
                stepping.run_inspector(cfg, "Adjust stepsize: ", dist_to_next,
                                       i, step_size_scaling(error));
            }
        }
    ...
    detray::pair<vector3_type, vector3_type> evaluate_dtds(
        ...
        DETRAY_DEBUG_HOST("-> evaluate dtds: "
                          << vector3_type{qop * vector::cross(t, b_field)});

        return detray::make_pair(vector3_type{qop * vector::cross(t, b_field)},
                                 t);
    }
    ...
    void advance_track(
        ...
        pos =
            pos + h * (sd.t[0u] + h_6 * (sd.dtds[0] + sd.dtds[1] + sd.dtds[2]));
        track.set_pos(pos);

        // Reference: Eq (82) of https://doi.org/10.1016/0029-554X(81)90063-X
        dir = dir +
              h_6 * (sd.dtds[0] + 2.f * (sd.dtds[1] + sd.dtds[2]) + sd.dtds[3]);
    ...
    void advance_jacobian(
        ...
        /// and dG/dT (calles dGdT). The second is given by the top 3 lines of
...

        {
...

            {
                ...
                // dk1/dqop1
                dkndqop[0u] =
                    dqopn_dqop[0u] * vector::cross(sd.t[0u], sd.b_first);

                // dk2/dqop1
                dkndqop[1u] =
                    dqopn_dqop[1u] * vector::cross(sd.t[1u], sd.b_middle) +
                    sd.qop[1u] * half_h *
                        vector::cross(dkndqop[0u], sd.b_middle);

                // dk3/dqop1
                dkndqop[2u] =
                    dqopn_dqop[2u] * vector::cross(sd.t[2u], sd.b_middle) +
                    sd.qop[2u] * half_h *
                        vector::cross(dkndqop[1u], sd.b_middle);
...

        {
            ...
            dFdt = h * dFdt;
            dGdt = dGdt + h_6 * (dkndt[0u] + 2.f * (dkndt[1u] + dkndt[2u]) +
                                 dkndt[3u]);
        }
        ...
        if constexpr (flags_v & static_cast<std::uint32_t>(
            ...
            if (cfg.use_field_gradient) {
                ...
                vector3_type r_fin =
                    r_ini + h * sd.t[0u] + h2 * 0.5f * sd.dtds[2u];

                matrix_type<3, 3> dBdr_ini =
                    evaluate_field_gradient(stepping, r_ini);
                matrix_type<3, 3> dBdr_mid =
                    evaluate_field_gradient(stepping, r_mid);
                matrix_type<3, 3> dBdr_fin =
                    evaluate_field_gradient(stepping, r_fin);
                ...
                dGdr = h_6 *
                       (dkndr[0u] + 2.f * (dkndr[1u] + dkndr[2u]) + dkndr[3u]);
            }
        } else {
    ...
    matrix_type<3, 3> evaluate_field_gradient(
        ...
        for (unsigned int i = 0; i < 3; i++) {
            ...
            const auto bvec1_tmp =
                stepping.field().at(dpos1[0], dpos1[1], dpos1[2]);
            vector3_type bvec1;
            ...
            const auto bvec2_tmp =
                stepping.field().at(dpos2[0], dpos2[1], dpos2[2]);
            vector3_type bvec2;
            ...
            assert(delta != 0.f);
            const vector3_type gradient =
                (bvec1 - bvec2) * (1.f / (2.f * delta));

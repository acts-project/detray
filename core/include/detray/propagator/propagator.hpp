...
struct propagator {
    ...
    requires concepts::is_state_of<actor_states_t, actor_chain_type>
        ...
        // structure. Indeed, the structure is always to run the update
        // once at the start, and then run it twice every time with the
        // actors in between which gives rise to the following structure:
        //
        // A = actors
        // N = navigation update
        // S = propagation step
        //
        // NASNANSNANSNANSNANSNANS...
        //   ^ Note the missing navigation update here.
        for (unsigned int i = 0; propagation.is_alive(); ++i) {
            if (i == 0 || i % 2 == 1) {
                is_init = false;
            }

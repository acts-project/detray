...
struct config {
    ...
    ///    first  entry: Minimal tolerance (~ position uncertainty on surface)
    ///    second entry: Maximal tolerance, which is used when still far away
    ///                  from surface
    std::array<scalar_t, 2u> mask_tolerance = {1e-5f * unit<scalar_t>::mm,

...
class single_store {
    ...
    /// @return the view on the underlying container - non-const
    DETRAY_HOST auto get_buffer(vecmem::memory_resource &resource,
                                vecmem::copy &cpy) -> buffer_type {
        return detray::get_buffer(get_data(), resource, cpy);
    }

    /// @return the view on the underlying container - const
    DETRAY_HOST auto get_buffer(vecmem::memory_resource &resource,
                                vecmem::copy &cpy) const -> const_buffer_type {
        return detray::get_buffer(get_data(), resource, cpy);
    }

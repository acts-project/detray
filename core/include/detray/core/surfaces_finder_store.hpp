...
struct surfaces_finder_store {
     ...
     */
    DETRAY_HOST surfaces_finder_store(vecmem::memory_resource& resource)
    ...
    template <typename surfaces_finder_data_t,
              std::enable_if_t<!std::is_base_of_v<vecmem::memory_resource,
                               ...
                               bool> = true>
    DETRAY_HOST_DEVICE surfaces_finder_store(surfaces_finder_data_t& finder_data)
...
struct surfaces_finder_store_data {
    // type definition
 ...
 */
template <typename surfaces_finder_t>
struct surfaces_finder_store_view {
    // type definition
    using surface_grid_t =
        typename surfaces_finder_t::surfaces_regular_circular_grid;

...
struct navigator {
  ...
  enum navigation_trust_level : int {
    ...
    e_fair_trust =
        1, // re-evaluate the distance & order of the (preselected) candidates
  ...
  bool status(state &navigation, const track<context> &track) const {
    ...
    if (not is_exhausted(surface_kernel) and
        update_kernel(navigation, surface_kernel, track, volume.surface_range(),
   ...
   */
  template <typename kernel_t> bool is_exhausted(const kernel_t &kernel) const {

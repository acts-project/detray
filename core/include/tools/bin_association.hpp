  ...
  if (not is_cylinder) {
    ...
    for (unsigned int bin_0 = 0; bin_0 < axis_0.bins(); ++bin_0) {
      for (unsigned int bin_1 = 0; bin_1 < axis_1.bins(); ++bin_1) {
        ...
        scalar r_add = absolute_tolerance
                           ? bin_tolerance[0]
        ...
        std::vector<point2> bin_contour =
            r_phi_polygon(r_borders[0] - r_add, r_borders[1] + r_add,
        ...
        // for (auto [is, s] : enumerate(surfaces.objects()))
        {
          ...
          // Usually one mask per surface, but design allows - a single
          // association  is sufficient though
    ...
    for (unsigned int bin_0 = 0; bin_0 < axis_0.bins(); ++bin_0) {
      for (unsigned int bin_1 = 0; bin_1 < axis_1.bins(); ++bin_1) {
        ...
        scalar z_add = absolute_tolerance
                           ? bin_tolerance[0]

...
struct regular {
  ...
  dindex_sequence zone_t(scalar v,
                         const array_type<neighbor_t, 2> &nhood) const {
    dindex_range nh_range = range(v, nhood);
    dindex_sequence sequence(
        static_cast<dindex_sequence::size_type>(nh_range[1] - nh_range[0] + 1),
        nh_range[0]);
    dindex m = 0;
...
struct irregular {
   ...
   **/
  dindex bin(scalar v) const {
    dindex ibin = std::lower_bound(boundaries.begin(), boundaries.end(), v) -
                  boundaries.begin();

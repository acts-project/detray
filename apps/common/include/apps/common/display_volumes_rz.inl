      ...
      if (argc > 6) {
        ...
        if (argc > 7) {
          ...
          if (detail_draw_option == 3) {
            ...
            for (dindex ir = 0; ir < r_axis.bins(); ++ir) {
              ...
              for (dindex iz = 0; iz < z_axis.bins(); ++iz) {
                ...
                auto v_dis = rectangle(z_borders[0], r_borders[0],
          ...
          if (detail_draw_option == 4) {
            ...
            for (dindex ir = 0; ir < r_axis.bins(); ++ir) {
              for (dindex iz = 0; iz < z_axis.bins(); ++iz) {
                ...
                auto rz = rectangle(z_borders[0], r_borders[0],
                                    z_borders[1] - z_borders[0],
          ...
          if (detail_draw_option == 5) {
            ...
            std::cout << "[detray] Drawing expanded volume grid around kernels"
                      << std::endl;
            rectangle(rz_det[2], rz_det[0], rz_det[3] - rz_det[2],
                      rz_det[1] - rz_det[0], 0);
            ...
            for (const auto &[vidx, be] : volume_extends) {
              ...
              std::cout << "     Volume " << vidx
                        << " with rmin/rmax = " << vol_rmin << ", " << vol_rmax
                        << std::endl;
              std::cout << "            " << vidx
                        << " with zmin/zmax = " << vol_zmin << ", " << vol_zmax
                        << std::endl;
              std::cout << "            " << vidx << " from  = (" << b[0]
                        << ", " << b[1] << ")" << std::endl;
              std::cout << "            " << vidx << " to    = (" << e[0]
                        << ", " << e[1] << ")" << std::endl;
              auto vis_vol = rectangle(vol_zmin, vol_rmin, vol_zmax - vol_zmin,
                                       vol_rmax - vol_rmin, 0);
            }
          }
        }
        ...
        if (base_draw_option == 1) {
          ...
          for (const auto &v : d.volumes()) {
            const auto &v_bounds = v.bounds();
            auto v_dis =
                rectangle(v_bounds[2], v_bounds[0], v_bounds[3] - v_bounds[2],

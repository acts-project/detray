...
int main(int argc, char **argv) {
    ...
    if (argc > 1) {
        ...
        if (first_arg == "-h" or first_arg == "--help") {
            std::cout << "[detray] Usage: 'display_volumes_rz detector_name "
            ...
            darray<scalar, 4> rz_det = {scalar_limit, 0., scalar_limit,
                                        -scalar_limit};
            ...
            auto attach = [](dmap<scalar, std::vector<dindex>> &attachments,
            ...
            auto sort_and_remove_duplicates =
                [](std::vector<dindex> &att) -> void {
                std::sort(att.begin(), att.end());
                att.erase(std::unique(att.begin(), att.end()), att.end());
            };

            std::cout << "[detray] Detector grid bins r_min "
                      << r_min_attachments.size() << std::endl;
            std::cout << "                            r_max "
                      << r_max_attachments.size() << std::endl;
            std::cout << "                            z_min "
                      << z_min_attachments.size() << std::endl;
            std::cout << "                            z_max "
                      << z_max_attachments.size() << std::endl;
            ...
            if (argc > 6) {
                ...
                if (argc > 7) {
                    ...
                    if (detail_draw_option == 1) {
                        std::cout << "[detray] Drawing grid lines."
                                  << std::endl;
                    ...
                    if (detail_draw_option == 2 and base_draw_option == 0) {
                        std::cout << "[detray] Drawing volume heat map."
                                  << std::endl;
                    ...
                    if (detail_draw_option == 3) {
                        std::cout << "[detray] Drawing volume grid."
                                  << std::endl;
                            ...
                            for (dindex iz = 0; iz < z_axis.bins(); ++iz) {
                                ...
                                auto v_dis =
                                    rectangle(z_borders[0], r_borders[0],
                                              z_borders[1] - z_borders[0],
                                              r_borders[1] - r_borders[0], 0);
                    ...
                    if (detail_draw_option == 4) {
                        std::cout << "[detray] Drawing grid colored"
                                  << std::endl;
                        rectangle(rz_det[2], rz_det[0], rz_det[3] - rz_det[2],
                                  rz_det[1] - rz_det[0], 0);
                        ...
                        for (dindex ir = 0; ir < r_axis.bins(); ++ir) {
                            for (dindex iz = 0; iz < z_axis.bins(); ++iz) {
                                ...
                                if (volume_colors.find(volume_index) ==
                                    volume_colors.end()) {
                                    volume_colors[volume_index] =
                                        random_color();
                                }
                    ...
                    if (detail_draw_option == 5) {
                        ...
                        std::cout << "[detray] Drawing expanded volume grid "
                                     "around kernels"
                                  << std::endl;
                        rectangle(rz_det[2], rz_det[0], rz_det[3] - rz_det[2],
                                  rz_det[1] - rz_det[0], 0);
                        ...
                        for (dindex ir = 0; ir < r_axis.bins(); ++ir) {
                            for (dindex iz = 0; iz < z_axis.bins(); ++iz) {
                                dindex volume_index = volume_grid.bin(ir, iz);
                                auto vol_itr =
                                    volume_extends.find(volume_index);
                        ...
                        std::cout << "[detray] Found " << volume_extends.size()
                                  << " individual volume extends." << std::endl;
                        for (const auto &[vidx, be] : volume_extends) {
                            ...
                            std::cout << "     Volume " << vidx
                                      << " with rmin/rmax = " << vol_rmin
                                      << ", " << vol_rmax << std::endl;
                            std::cout << "            " << vidx
                                      << " with zmin/zmax = " << vol_zmin
                                      << ", " << vol_zmax << std::endl;
                            std::cout << "            " << vidx << " from  = ("
                                      << b[0] << ", " << b[1] << ")"
                                      << std::endl;
                            std::cout << "            " << vidx << " to    = ("
                                      << e[0] << ", " << e[1] << ")"
                                      << std::endl;
                            auto vis_vol = rectangle(vol_zmin, vol_rmin,
                                                     vol_zmax - vol_zmin,
                                                     vol_rmax - vol_rmin, 0);
                        }
                    }
                ...
                if (base_draw_option == 1) {
                    ...
                    for (const auto &v : d.volumes()) {
                        const auto &v_bounds = v.bounds();
                        auto v_dis = rectangle(v_bounds[2], v_bounds[0],
                                               v_bounds[3] - v_bounds[2],
                                               v_bounds[1] - v_bounds[0], 0);
                        v_dis->fill(true);
                        if (not v.empty()) {

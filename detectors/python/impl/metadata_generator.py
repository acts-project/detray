...
class metadata:
    ...
    def set_algebra_plugin(self, plugin: Algebra, precision: Type):
        self.algebra = plugin
        self.precision

        if self.algebra is Algebra.ANY:
    ...
    def add_shape(self, shape: Shape):
        if not shape in self.shapes:
            self.logger.info(f'Adding surface shape "{shape.specifier}"')
    ...
    def add_material(self, mat: Material):
        if mat not in self.materials:
            if mat is Material.SLAB or mat is Material.ROD or mat is Material.RAW:
                self.logger.info(f'Adding material type "{mat.specifier}"')
            else:
                shape_secifier = mat.param["shape"].specifier
                self.logger.info(
                    f'Adding material type "{mat.specifier}<{shape_secifier}>"'
                )
    ...
    def add_accel_struct(self, accel: Accelerator, obj_type: str = "sensitive"):
        if "grid" in accel.specifier:
            shape_secifier = accel.param["shape"].specifier
            self.logger.info(
                f"Adding accel. struct ({obj_type}): {accel.specifier}<{shape_secifier}>"
            )
        else:
            self.logger.info(f"Adding accel. struct ({obj_type}): {accel.specifier}")
...
{self.__tabs()}    single_store<dtransform3D<algebra_type>, vector_t, geometry_context>;"
        ...
        if md.acceleration_structs:
            ...
            self.__declare_type_enum(
                ...
                self.accel_specifiers,
                md.id_base,
                extra_items=[["default", "e_brute_force"]],
            )
            self.__lines(2)
            self.__define_enum_stream_op(
                "accel_id",
        ...
        for i, (accel, obj_set) in enumerate(accel_types.items()):
            # Only add the brute force methods where no other accelerator is
            # available
            if accel is Accelerator.BRUTE_FORCE.specifier:
                continue

            n_accel_links = i
            for geo_obj in obj_set:
                if geo_obj not in accel_link_types:
                    self.__put(f"e_{geo_obj} = {i}u,\n")
                    accel_link_types.append(geo_obj)
                else:
                    self.logger.error(
                        f'Multiple acceleration structure links defined for geometry object: "{geo_obj}"\nEach geometry object type must be associated with exactly one type of links per volume:\n{accel_types}'
                    )

        # Now add the brute force link for the leftover object types (link 0)
        obj_set = accel_types[Accelerator.BRUTE_FORCE.specifier]
        for geo_obj in obj_set:
            if geo_obj not in accel_link_types:
                self.__put(f"e_{geo_obj} = 0u,\n")
                accel_link_types.append(geo_obj)

        # Set the size of the accelerator multi-link
        self.__put(f"e_size = {n_accel_links + 1}u,\n")
        self.__put(f"e_all = e_size,\n")
 ...
 */"
    ...
    def __declare_surface_descriptor(self, md: metadata):
        self.__put("using transform_link = typename transform_store<>::single_type;\n")
    ...
    def __define_enum_stream_op(self, specifier, items, extra_items=[]):
        ...
        for i, v in enumerate(items + extra_items):
            ...
            self.indent = self.indent + 1
            self.__put(f'os << "{sub_specifier}";\n')
            self.__put(f"break;\n")
            self.indent = self.indent - 1

        # Add the default case
        self.__put(f"default:\n")
        self.indent = self.indent + 1
        self.__put(f'os << "invalid";\n')
        self.indent = self.indent - 1
...
#include "detray/geometry/surface_descriptor.hpp"\n'
    ...
    def __declare_multi_store(
        ...
        if is_regular:
            ...
            type_list = []
            if "brute_force_t" in types:
                types.remove("brute_force_t")

...
namespace detray::actsvg_visualization {
    ...
    inline std::array<detray::scalar, 3> rotation_matrix_to_euler_angles(const matrix_t& mat){
        float a = std::sqrt(mat[0][0] * mat[0][0] + mat[1][0] * mat[1][0]);
        // Checking if it is singular.
        if (a < 1e-6)
            return {std::atan2(-mat[1][2], mat[1][1]), std::atan2(-mat[2][0], a), 0};

        return {std::atan2(mat[2][1], mat[2][2]), std::atan2(-mat[2][0], a), std::atan2(mat[1][0], mat[0][0])};
    }
    ...
    inline auto convert_transform(const transform_t& d_transform){
        ...
        ret._tr = {static_cast<actsvg::scalar>(translation[0]), static_cast<actsvg::scalar>(translation[1])};
        ret._rot = {
    ...
    inline proto_surface convert_mask(const mask_t& m){
        ...
        constexpr auto is_cylinder2D = std::is_same_v<shape_t, detray::cylinder2D<>>;
        constexpr auto is_rectangle2D = std::is_same_v<shape_t, detray::rectangle2D<>>;
        constexpr auto is_ring2D = std::is_same_v<shape_t, detray::ring2D<>>;
        constexpr auto is_trapezoid2D = std::is_same_v<shape_t, detray::trapezoid2D<>>;
        ...
        {
            point3 av = {static_cast<actsvg::scalar>(dv[0]), static_cast<actsvg::scalar>(dv[1]), static_cast<actsvg::scalar>(dv[2])};
    ...
    proto_surface convert_surface(const detray::surface<detector_t>& d_surface, const typename detector_t::geometry_context& context)
    {

...
template <typename detector_t, typename names_t>
class svg_converter {
    ...
    constexpr svg_converter(const detector_t& det, const names_t& names)
        : _detector{det}, _names{names} {}

    /// @returns an svg of the detector's surface in x-y view.
    inline auto xy(const typename detector_t::geometry_context& context, const detray::surface<detector_t>& surface, const detector::detector_options& options = default_options) const{
        return to_svg(context, _xy_view, surface, options.v_options.s_options, options.v_options.p_options, "");
    }

    /// @returns an svg of the detector's volume in x-y view.
    inline auto xy(const typename detector_t::geometry_context& context, const detray::detector_volume<detector_t>& volume, const detector::detector_options& options = default_options) const{
        return volume::to_svg(context, _xy_view, _detector, volume, options.v_options, "");
    }

    /// @returns an svg of the detector in x-y view.
    ...
    /// @returns an svg of the detector's surface in z-r view.
    inline auto zr(const typename detector_t::geometry_context& context, const detray::surface<detector_t>& surface, const detector::detector_options& options = default_options) const{
        return surface::to_svg(context, _zr_view, surface, options.v_options.s_options, options.v_options.p_options, "");
    }

    /// @returns an svg of the detector's volume in z-r view.
    inline auto zr(const typename detector_t::geometry_context& context, const detray::detector_volume<detector_t>& volume, const detector::detector_options& options = default_options) const{
        return volume::to_svg(context, _zr_view, _detector, volume, options.v_options, "");
    }

    /// @returns an svg of the detector in z-r view.

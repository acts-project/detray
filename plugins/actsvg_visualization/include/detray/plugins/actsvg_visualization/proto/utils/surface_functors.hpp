    ...
    inline auto link_dir(
        ...
        for (const auto& desc : volume_link.surface_lookup()) {
            ...
            if (surface.is_portal()) {
                if (auto radius =
                        surface.template visit_mask<outer_radius_functor>()) {
...
struct to_proto_surface_functor {
    ...
    DETRAY_HOST inline auto operator()(
        ...
        const detray::surface<detector_t>& d_surface) const {
        const auto& m = mask_group[index];
    ...
    auto inline to_proto_surface(
        const typename detector_t::geometry_context& context,
        const detray::surface<detector_t>& d_surface, const polygon_t,
    ...
    template <typename detector_t, typename bounds_t, bool kRadialCheck,
              template <typename> class intersector_t>
    auto inline to_proto_surface(
        const typename detector_t::geometry_context& context,
        const detray::surface<detector_t>& d_surface,
        const detray::cylinder2D<kRadialCheck, intersector_t>& shape,
        const bounds_t& bounds) const {
        // Rotation for circular objects is currently not supported.
        assert(d_surface.transform.rotation(context) ==
               typename detector_t::transform3{});
        // Only translation one z axis is supported.
        assert(d_surface.transform.translate(context).x() == 0 &&
               d_surface.transform.translate(context).y() == 0);
        ...
        auto hz = (phz - nhz) / 2 +
                  static_cast<actsvg::scalar>(d_surface.center(context)[2]);
    ...
    auto to_proto_surface(const typename detector_t::geometry_context& context,
                          const detray::surface<detector_t>& d_surface,
                          const detray::ring2D<>& shape,
                          const bounds_t& bounds) const {
        // Rotation for circular objects is currently not supported.
        assert(d_surface.transform.rotation(context) ==
               typename detector_t::transform3{});
    ...
    auto inline to_proto_surface(
        const typename detector_t::geometry_context& context,
        const detray::surface<detector_t>& d_surface,
        const detray::annulus2D<>& shape, const bounds_t& bounds) const {
        // Rotation for circular objects is currently not supported.
        assert(d_surface.transform.rotation(context) ==
               typename detector_t::transform3{});
    ...
    /// @brief Sets the vertices of the proto surfaces type to be equivalent to
    /// the detray shape.

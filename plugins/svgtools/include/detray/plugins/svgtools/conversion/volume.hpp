...
const std::vector<detray::surface<detector_t>>& d_surfaces) {
    ...
    for (const auto& item : d_surfaces){
        if (item.is_portal()){
            auto portal = svgtools::conversion::portal<point3_container_t>(context, detector, item);
            portals.push_back(portal);
        }
        else{
            auto surface = svgtools::conversion::surface<point3_container_t>(context, item);
...
const detray::detector_volume<detector_t>& d_volume) {
    ...
    const auto descriptors = svgtools::utils::surface_lookup(detector, d_volume);
    for (const auto& desc : descriptors){
        surfaces.push_back(detray::surface{detector, desc});
    }
    return svgtools::conversion::volume<point3_container_t>(context, detector, surfaces);
}

...
class illustrator{
    ...
    illustrator(const detector_t& detector, const typename detector_t::name_map& name_map) 
    : _detector{detector}, _name_map{name_map} {}

    illustrator(const detector_t& detector, const typename detector_t::name_map& name_map, const styling::style& style) 
    ...
    inline auto draw_surfaces(const std::string& identification, const typename detector_t::geometry_context& context, const iterator_t& indices, const view_t& view) const {
        actsvg::svg::object ret;
        ret._tag = "g";
        ret._id = identification;
        for (const auto index : indices){
            const auto svg = draw_surface(identification + "_surface" + std::to_string(index), context, index, view);
            ret.add_object(svg);
        }
        return ret;
    }
    ...
    template <typename iterator_t, typename view_t>
    inline auto draw_volumes(const std::string& identification, const typename detector_t::geometry_context& context, const iterator_t& indices, const view_t& view) const {
        actsvg::svg::object ret;
        ret._tag = "g";
        ret._id = identification;
        for (const auto index : indices){
            const auto svg = draw_volume(identification + "_volume" + std::to_string(index), context, index, view);
            ret.add_object(svg);
        }
        return ret;
    }
    ...
    template <typename view_t>
    inline auto draw_detector(const std::string& identification, const typename detector_t::geometry_context& context, const view_t& view) const {
        actsvg::svg::object ret;
        ret._tag = "g";
        ret._id = identification;
